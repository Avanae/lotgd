...
function racehuman_dohook($hookname, $args)
{
    ...
    switch ($hookname) {
        ...
        case "setrace":
            if ($session['user']['race'] == $race) {
                output("`&As a human, your size and strength permit you the ability to effortlessly wield weapons, tiring much less quickly than other races.`n`^You gain an extra forest fight each day!");
                if (is_module_active("cities")) {
                    if (
        ...
        case "stablelocs":
            ...
            $args[$city] = Translator::getInstance()->sprintfTranslate("The Village of %s", $city);
            Translator::getInstance()->setSchema();
            break;
    }
    return $args;
