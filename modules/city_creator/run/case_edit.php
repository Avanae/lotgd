...
if ($cityid > 0) {
    ...
    $sql = "SELECT * FROM " . Database::prefix('cities') . " WHERE cityid = $cityidEsc";
...
// **Behoud de originele form array**
$form = array(
		'City Details,title',
...
// **Module prefs ophalen via Database class**
$sql = "SELECT formalname, modulename FROM " . Database::prefix('modules') . " WHERE infokeys LIKE '%|prefs-city|%' ORDER BY formalname";
$result = Database::query($sql);
while ($row2 = Database::fetchAssoc($result)) {
    ...
    if (!empty($info['prefs-city'])) {
        ...
        $sql2 = "SELECT setting, value FROM " . Database::prefix('module_objprefs') . " WHERE modulename = '$modulename' AND objtype = 'city' AND objid = $cityidEsc";
