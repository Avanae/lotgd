...
if (!empty($cityid)) {
    // Haal city data op
    $sql = "SELECT * FROM " . Database::prefix('cities') . " WHERE cityid = ?";
    $result = Database::query($sql, [$cityid]);
    ...
    '`^Leave empty to travel from this city to anywhere&#44; or enter city names to restrict routes.`nTo stop all travel you can either block the travel link or block the \'cities\' module.`n`#Separate each city name with a comma!,note',

    'City Text,title',
    'villtitle'=>'Title:,string,60',
    'villtext'=>'Main Description:,citytextarea,30',
    ...
    'merconecompanion'=>'One offers To join you:,text',
    ...
    '`^If you\'re blocking all modules then enter the module names above which you want to unblock. For example unblock the \'cities\' module if you want travel to work.`n`nIf you\'re *not* blocking all modules then enter the module names above which you do want to block.`n`n`#Separate the names with a comma!,note',
	...
	$sql = "SELECT formalname, modulename
        FROM " . Database::prefix('modules') . "
        WHERE infokeys LIKE '%|prefs-city|%'
        ORDER BY formalname";
$result = Database::query($sql);
while ($row2 = Database::fetchAssoc($result)) {
    ...
    if (count($info['prefs-city']) > 0) {
        $form[] = $formalname . ',title';
        foreach ($info['prefs-city'] as $key => $val) {
            if (strpos($val, ',title') !== false) {
                $val = '`^`i' . str_replace(',title', '`i,note', $val);
            }

            $x = is_array($val) ? explode('|', $val[0]) : explode('|', $val);
            $form[$modulename . '-' . $key] = $x[0];
            $row[$modulename . '-' . $key] = $x[1] ?? '';
        }

        // Haal module object prefs op
        $sql2 = "SELECT setting, value
                 FROM " . Database::prefix('module_objprefs') . "
                 WHERE modulename = ? AND objtype = 'city' AND objid = ?";
        $result2 = Database::query($sql2, [$modulename, $cityid]);
        while ($row3 = Database::fetchAssoc($result2)) {
