...
$races_array = [
    ['`7Gray Ferret', 4800, 3, 55, 0, 1, 0, 'A small gray ferret popular as pets among the younger crowd.', 'Emerging from your backpack with a yawn, your little ferret is ready for the day\'s travels.', 'Your ferret hides in your pack as you wander about the village.', 'Your ferret goes chasing after a grasshopper as you stroll through the gardens.', 'Your ferret buries itself in your backpack as the battle begins.',0,0,0,0,0,'All'],
    ['`&White Rabbit', 4500, 2, 100, 0, 1, 0, 'A small rabbit with a soft white coat, popular among children.', 'Your rabbit hops around your feet as you prepare for the new day at hand.', 'As you wander through the village, you hold your pet rabbit in your arms.', 'Your pet rabbit hops among the flower patches, searching for a meal.', 'With a squeal, your rabbit hides among the bushes as the enemy draws closer.',0,0,0,0,0,'All'],
    ['`)Black Rat', 3150, 2, 67, 0, 0, 0, 'A small, slightly diseased black rat. Perfect for any social occasion.', 'Not one for mornings, your little black rat remains inside your backpack.', 'Villagers keep a cautious distance from you and your black rat.', 'Your little black rat hunts for food scraps and insects among the flower beds.', 'Terrified, your little black rat buries itself in your backpack as the battle begins.',0,0,0,0,0,'All'],
    ['`qBrown Rat', 3200, 2, 85, 0, 0, 0, 'A small brown rat, common to many alleyways and shops.', 'New day dawning, your pet rat peeks out from your pack with a somewhat disinterested look upon its face.', 'Fearful of the villagers, your little brown rat hides in your backpack.', 'Your little brown rat hides in your pack as a fairy buzzes by.', 'Terrified, your little brown rat buries itself in your backpack as the battle begins.',0,0,0,0,0,'All'],
];
...
if ($sop == 'install') {
    ...
    for ($i=0; $i<$count; $i++) {
        if (($races["race$i"] ?? 0) == 1 || $allraces == 1) {
            $race_values = [];
            foreach ($races_array[$i] as $value) {
                $race_values[] = "'".addslashes($value)."'";
            }
            $race_sql = implode(',', $race_values);

            $sql = "INSERT INTO ".db_prefix('races')." ($fields) VALUES ($race_sql)";
    ...
    $races_checklist = '';
    for ($i=0; $i<$count; $i++) {
        $races_checklist .= ($i>0?',':'')."race$i,".appoencode($races_array[$i][0]);
    }
    ...
    $form = [
        ...
        '`^Aren\'t you glad I put this at the top? Heh.,note',
        'races' => 'Races:,checklist'.$races_checklist
    ];
